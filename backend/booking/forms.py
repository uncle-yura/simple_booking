    ...
    def __init__(self, *args, **kwargs):
        ...
        for master in query_set:
            ...
            if (
                ...
                and not (
                    timetable == "M"
                    and master.get_uniq_clients().filter(id__exact=request.user.profile.id).count()
                    > 0
                )
                and not (timetable == "V" and request.user.profile.orders.count() > 0)

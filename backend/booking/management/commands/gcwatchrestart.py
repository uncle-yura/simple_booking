...
class Command(BaseCommand):
    help = "Stop watch for changes to Events resources."

    def handle(self, *args, **options):
        masters = Profile.objects.filter(user__groups__name="Master")
        for master in masters:
            result = master.stop_watch_calendar()
            self.stdout.write(
                f"Profile #{master.id} calendar watcher status: "
                + f"{'Stopped' if result else 'Error'}"
            )

            result = master.start_watch_calendar(force=True)
            self.stdout.write(
                f"Profile #{master.id} calendar watcher status: "
